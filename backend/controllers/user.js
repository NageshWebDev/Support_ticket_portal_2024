...
exports.getUserTickets = async (req, res, next) => {
  ...
  try {
    const userRole = await getUserRole(userId);
    const query = userRole === "admin" ? null : { userId };
...
exports.getUserTicket = async (req, res, next) => {
  ...
  try {
    ...
    const ticketDetails = await Ticket.findOne(query).populate(
      "userDetails",
      "name email -_id"
    );

    console.log("🚀 ~ exports.getUserTicket= ~ ticketDetails:", ticketDetails);
...
exports.createUserTicket = async (req, res, next) => {
  ...
  if (userRole === "admin") userId = body.userId;
